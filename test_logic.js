...
const { S, pickInfo, myManager, isAdmin, boardRulesNote, calcPlayerPoints, calcTeamPoints, computeScores,
        ...
        fixtureWindows, matchweeksOf, WINDOW_DEFAULTS, shortlistCoverage, coverageHint, applyVisibleOrder,
        currentSeasonFor, seasonOptions, seasonLabel, backtestSeason, pickReconciliation, squadCheck,
        ...
        fetchCompetitionPool, fetchCompetitionFixtures, pickCurrentClub, latestTransfers, applyTransfers, arrivalRow, knownApiPos, transferFloor, transferSince, MAX_TRANSFER_REMOVALS, MAX_TRANSFER_ARRIVALS, compKeyOf, competitionKey,
        ...
        playerBreakdown, playerPoints, passAccuracyPct, rawStatsOf, normaliseEvents, concededMinutes, concededMinutesByTeam, onPitchWindow, concededWhileOn, ON_PITCH_END, suspendedNext, yellowBanCount, yellowWindow, injuryFeedNote, effectiveCaptain, resilientWrite,
        ...
        statsScopedRows, sumStatKey, statSorts, AVERAGED_STATS, sumMinutes, formAvg, formLog,
...
/* The priority itself, which the screen and the resolver now share. A league
   ...
   auto-window mode nothing ever seeds it -- so the queue has to be derivable
   from the standings alone or there is nothing to show. */
{
  const ids = ["a", "b", "c", "d"];
...
/* Every API path the app asks for must be one the proxy will pass on.

   Nearly shipped: fixtures/events was added to app.js and not to the edge
   function's ALLOWED set. The proxy answers an unlisted path with a 400, and
   the caller swallows its own failures on purpose -- so the app would have
   scored every match as though no events existed, while the Python pull,
   which talks to the API directly, scored them correctly. Two writers
   disagreeing again, from one forgotten line, and nothing on screen to say
   so.

   Read out of both files as text rather than asserted against a list written
   here, because a third copy of the same set is the thing that goes stale. */
{
  const fs = require("fs"), path = require("path");
  const src = fs.readFileSync(path.join(__dirname, "app.js"), "utf8");
  const fn = fs.readFileSync(path.join(__dirname, "supabase", "functions",
    "api-football", "index.ts"), "utf8");
  const allowed = new Set(
    (fn.match(/const ALLOWED = new Set\(\[([\s\S]*?)\]\)/)?.[1] || "")
      .split("\n").map((l) => l.match(/"([^"]+)"/)?.[1]).filter(Boolean));
  const asked = new Set(
    [...src.matchAll(/apiFootball\(\s*\w+\s*,\s*"([^"]+)"/g)].map((m) => m[1]));
  check("the app asks for paths at all", asked.size > 0, true);
  check("every path the app calls is allowed by the proxy",
    [...asked].filter((p) => !allowed.has(p)), []);
}

/* Goals conceded WHILE ON THE PITCH, against the shared case table.

   The cases live in test/fixtures/on_pitch.json and the Python suite reads
   the same file. app.js and daily_pull.py both write this stat, and a stat
   two writers disagree about is exactly the bug it replaces -- so they get
   one specification rather than two copies of it that drift the first time
   somebody fixes only one. If these two suites ever disagree, one of the
   writers has moved and the other has not. */
{
  const spec = JSON.parse(require("fs").readFileSync(
    require("path").join(__dirname, "test", "fixtures", "on_pitch.json"), "utf8"));
  check("the shared case table is intact", spec.cases.length >= 10, true);
  check("both languages agree on the final-whistle sentinel",
  ...
  // "og_swap" files an own goal under the OTHER side -- the second thing the
  // feed might do, and the second thing nothing may depend on.
  const asApi = (list, swap, ogSwap, home, away) => list.map((e) => {
    const on = swap ? e.off : e.on, off = swap ? e.on : e.off;
    const isOg = /own goal/i.test(e.detail || "");
    const team = (ogSwap && isOg) ? (e.team === home ? away : home) : e.team;
    return {
  ...
  const run = (c, label) => {
    const norm = normaliseEvents(asApi(c.events, c.swap, c.og_swap, c.home, c.away));
    for (const p of c.players) {
      const window = c.events.length
        ? onPitchWindow(p.id, norm, !p.substitute) : null;
      check(`${label} · ${p.id}: on-pitch window`, window, p.expect.window);
      const mins = concededMinutes(norm, p.team, c.home, c.away,
        c.score.home, c.score.away);
      const on = concededWhileOn(window, mins);
      const club = p.team === c.home ? c.score.away : c.score.home;
      const conceded = on != null ? on : club;
      check(`${label} · ${p.id}: goals conceded`, conceded, p.expect.conceded);
      check(`${label} · ${p.id}: clean sheet`,
        (p.minutes >= 60 && conceded === 0) ? 1 : 0, p.expect.clean_sheet);
    }
  };

  /* A goal event naming neither side must be IGNORED, not charged to
     whoever happened to be at home. `team === homeId ? awayId : homeId`
     sends every unrecognised value -- a null, a shootout, a competition
     mix-up -- to the home team, which is one of the two ways a goalkeeper
     who won 4-0 was charged a goal conceded. */
  {
    const ev = [
      { type: "Goal", detail: "Normal Goal", time: { elapsed: 20 }, team: { id: 10 } },
      { type: "Goal", detail: "Normal Goal", time: { elapsed: 70 }, team: { id: null } },
    ];
    const t = concededMinutesByTeam(normaliseEvents(ev), 10, 20, 1, 0);
    check("a goal naming neither side is not charged to the home team", t[10].length, 0);
    check("...and the goal that does name a side still counts", t[20].length, 1);
  }

  for (const c of spec.cases) run(c, c.name);
  run(spec.unplaceable, "unplaceable");
  run(spec.no_events, "no events");
}
...
/* Every stat the league SCORES can be sorted and filtered on.

   Reported from the app: "the watchlist and player search view does not have
   all of the scoring stats available to filter on." The options were a
   hand-written list of eleven legacy match_stats columns, frozen at whatever
   the World Cup league happened to score -- so key passes, pass accuracy,
   shots on target, dribbles, fouls drawn and goals conceded could not be
   ranked by at all. They were never missing; they were never offered.

   Driven off scoringRules() now, so the question this asks is the one that
   matters: is there a rule this league scores that its own player lists
   cannot rank by? */
{
  const keys = statSorts().map(([k]) => k);
  check("the sort list leads with the fantasy total", keys[0], "points");
  check("every scored stat can be sorted on",
    scoringRules().map((r) => r.stat).filter((k) => !keys.includes(k)), []);
  check("...and each one is offered under a readable name",
    statSorts().filter(([k, lbl]) => !lbl || lbl === k).map(([k]) => k), []);
  /* A league can carry two rules on one stat -- minutes >=1 and minutes >=60,
     which this league's rules do -- and the same words twice in a dropdown
     reads as a bug rather than as two rules. */
  check("a stat scored by two rules is offered once", keys.length, new Set(keys).size);
  /* The eleven labels the list already had, word for word. Driving the list
     off the catalogue gained every missing stat and quietly cost these their
     names -- the catalogue writes for the scoring editor and describes ONE
     event ("Goal", "Save (GK)", "Defensive action (tackle/block/int)"),
     which is right there and wrong in a dropdown that ranks a season. */
  const byKey = Object.fromEntries(statSorts());
  check("the labels a season-long list wants, not a rule editor's",
    ["goals.total", "goals.assists", "defensive_actions", "clean_sheet",
     "goals.saves", "motm", "cards.yellow", "cards.red"].map((k) => byKey[k]),
    ["Goals", "Assists", "Def. actions", "Clean sheets",
     "Saves", "MOTM", "Yellow cards", "Red cards"]);
}

/* Totals come from the SAME map the scoring engine reads.

   sumStatKey used to read r[key] straight off the row, which works only for
   the eleven legacy columns. Anything living in `raw` -- everything the
   catalogue has gained since -- totalled to zero, so a stat could appear in
   the dropdown and then rank the entire league at nil. */
{
  const rows = [
    { appeared: true, minutes: 90, goals: 1, raw: { "goals.total": 1, "passes.key": 3,
      "passes.accuracy": 40, "passes.total": 50, "shots.on": 2 } },
    { appeared: true, minutes: 90, goals: 0, raw: { "goals.total": 0, "passes.key": 1,
      "passes.accuracy": 45, "passes.total": 50, "shots.on": 0 } },
  ];
  check("a raw-map stat totals rather than reading zero", sumStatKey(rows, "passes.key"), 4);
  check("...and so does another", sumStatKey(rows, "shots.on"), 2);
  check("a legacy column still totals, for a saved sort preference",
    sumStatKey(rows, "goals"), 1);
  /* Percentages are AVERAGED. Summing pass accuracy ranks whoever played
     most games -- three matches at 90% would total 270 and beat one at 95 --
     which is not a worse sort, it is a different statistic wearing the right
     label. 80% and 90% average to 85. */
  check("pass accuracy is averaged, not added", sumStatKey(rows, "passes.pct"), 85);
  check("...and the averaged set says which stats those are",
    AVERAGED_STATS.has("passes.pct"), true);
  check("an averaged stat with no appearances is 0, not NaN",
    sumStatKey([], "passes.pct"), 0);
}
...
/* ...and once there IS a fixture calendar, "this round" is the round the
   line-up is FOR, not the newest one with results. Between two matchweeks
   those differ, and the home pager's This round toggle was tallying the
   finished round's points against the squad picked for the next one.
   Reported from the app. */
{
  const DAY = 24 * 3600e3, now = Date.now();
  const iso = (t) => new Date(t).toISOString();
  const cal = (r3At) => [
    { round: "Regular Season - 1", home: "A", away: "X", kickoff_utc: iso(now - 21 * DAY) },
    { round: "Regular Season - 2", home: "A", away: "Y", kickoff_utc: iso(now - 14 * DAY) },
    { round: "Regular Season - 3", home: "A", away: "Z", kickoff_utc: iso(r3At) },
  ];
  // Round 3 kicked off an hour ago: the line-up on screen is round 3's.
  S.fixtures = cal(now - 3600e3);
  check("the round in play is the round the line-up is for",
    lineupRoundNo(matchweeksOf(S.fixtures), now), 3);
  // Round 3 is a week away: still round 3's line-up, and nothing played in it.
  S.fixtures = cal(now + 7 * DAY);
  check("between matchweeks it is the round about to be played",
    lineupRoundNo(matchweeksOf(S.fixtures), now), 3);
  const gap = managerHistory("m1");
  check("...so the current lineup's round is the one it was picked for",
    gap.curRound, 3);
  check("...which nobody has played, so there is no this-round total",
    gap.curRoundPlayed, false);
  check("...and no player carries the finished round's points into it",
    gap.current.items.map((i) => i.roundPts), [0, 0]);
  check("...while the season total is untouched",
    gap.current.items.map((i) => i.pts), [12, 5]);
  // A calendar that cannot place a round says nothing, and the old
  // count-the-club's-games answer stands -- cups, and the legacy pool.
  check("a cup's round names place nothing",
    lineupRoundNo(matchweeksOf([{ round: "Round of 16", home: "A", away: "B",
      kickoff_utc: iso(now + DAY) }]), now), null);
  check("...and a season already over places nothing either",
    lineupRoundNo(matchweeksOf(cal(now - 7 * DAY)), now), null);
  S.fixtures = [];
  check("with no calendar the newest round with results is still used",
    managerHistory("m1").curRound, 2);
}
...
(async () => {
  ...
  /* Trade window between MW1 and MW2: opens mw1b + 3h, closes mw2a - 24h.
     THE THREE HOURS ARE MEASURED FROM A KICK-OFF, not a final whistle, which
     is the whole reason the default is not 1. `last` is the last KICK-OFF of
     the matchweek, so an hour after it is roughly half-time of the round's
     closing game -- trading reopened while the week it was settling was
     still being played. Three clears ninety minutes, half-time and stoppage.

     These assertions used to say 1h. They were not wrong about the code;
     they were faithful to a default that was wrong, which is the more
     dangerous kind of test. */
  ...
  check("trade window opens 3h after MW1's last KICK-OFF",
    wGap.tradeWindow.openAt, mw1b + 3 * awH);
  check("...which is the documented default, not a number typed twice",
    WINDOW_DEFAULTS.tradeOpenAfterH, 3);
  /* The lineup lock keeps its 1, and that is not an inconsistency: it is
     measured from the FIRST kick-off, where an hour before is exactly right.
     Both were a bare `?? 1` seven lines apart, which is how one of them
     stayed wrong. */
  check("the lineup lock is still an hour, measured from the other end",
    WINDOW_DEFAULTS.lineupLockBeforeH, 1);
  ...
  check("trade closed while the last game of MW1 is still on",
    fixtureWindows(awFx, mw1b + 30 * 60e3).tradeOpen, false);
  check("...and still closed an hour in, which is about half-time",
    fixtureWindows(awFx, mw1b + awH + 60e3).tradeOpen, false);
  check("...and still closed at two hours, around the final whistle",
    fixtureWindows(awFx, mw1b + 2 * awH).tradeOpen, false);
  check("trade opens once three hours have passed",
    fixtureWindows(awFx, mw1b + 3 * awH + 60e3).tradeOpen, true);
  // A league that set its own number keeps it, in both directions.
  check("an explicit setting still wins over the default",
    fixtureWindows(awFx, mw1b + awH + 60e3, { tradeOpenAfterH: 1 }).tradeOpen, true);
  check("...including a longer one",
    fixtureWindows(awFx, mw1b + 4 * awH, { tradeOpenAfterH: 6 }).tradeOpen, false);
  ...
  /* Has a club's match actually been PLAYED OUT? The one question behind
     every substitution and every captain armband, and the one this app got
     wrong in both directions inside a week.

     Reported from the app: a captain's match had finished, he had not played,
     and neither did a substitute come on nor did the armband pass to the
     vice. The cause was `f.status`, which is written into the fixture list
     once by the admin's "Load competition from API" button and never updated
     by any puller -- so a league loaded before the season says "not started"
     about every match, for ever. The banner three inches up the screen said
     FT, because it already applied the elapsed rule and this did not.

     Elapsed time ALONE is not the answer either, which is the half I missed
     first time: a postponed match keeps its original kick-off for ever, so a
     pure clock would rule two and a half hours later that a game nobody
     played had finished and mark everyone in it absent. It takes evidence —
     stats exist only because somebody played. */
  const kickAt = Date.UTC(2026, 7, 22, 14, 0);
  const notStarted = { status: "NS", kickoff_utc: new Date(kickAt).toISOString() };
  check("a status of FT is taken at its word, whatever the clock says",
    matchIsOver({ status: "FT", kickoff_utc: new Date(kickAt).toISOString() }, kickAt - 3600e3, false), true);
  check("still being played is not finished",
    matchIsOver(notStarted, kickAt + 3600e3, true), false);
  check("time is up and the stats prove it was played",
    matchIsOver(notStarted, kickAt + MATCH_MS + 60e3, true), true);
  check("time is up but NOTHING was ever played — a postponement stays open",
    matchIsOver(notStarted, kickAt + MATCH_MS + 7 * 86400e3, false), false);
  check("a fixture with no kick-off at all cannot be judged",
    matchIsOver({ status: "NS" }, kickAt + MATCH_MS, true), false);
  /* kickoff_utc arrives as a NUMBER from some paths, and Date.parse of a
     number is NaN -- the exact trap kickoffMs() was written for. */
  check("a numeric kick-off is understood, not silently NaN",
    matchIsOver({ status: "NS", kickoff_utc: kickAt }, kickAt + MATCH_MS + 1, true), true);
  /* MONOTONIC, which is what keeps a settled round settled: every input only
     moves false -> true, so a past round can never be re-opened and a
     substitute who counted stays counted. Checked across the boundary rather
     than asserted. */
  check("once over, it stays over — a week later, a year later",
    [kickAt + MATCH_MS + 1, kickAt + 30 * 86400e3, kickAt + 400 * 86400e3]
      .map((t) => matchIsOver(notStarted, t, true)), [true, true, true]);
  check("...and it is never over before its time",
    [kickAt - 1, kickAt, kickAt + MATCH_MS - 1].map((t) => matchIsOver(notStarted, t, true)),
    [false, false, false]);
  ...
  /* A player in two squads at once -- what the feed looks like around a
     transfer, with the new club holding him and the old one not yet letting
     go. Team order used to settle it, which is not recency and not anything.
     Observed live: "T. Awoniyi: kept Nottingham Forest, also listed at
     Coventry", on the same pull that reported nobody had changed club. */
  {
    const forest = { teamId: 65, team: "Nottingham Forest", entry: { player_id: "api_1" } };
    const cov = { teamId: 1798, team: "Coventry", entry: { player_id: "api_1" } };
    const opts = [forest, cov];
    const tx = [
      { date: "2024-07-01", teams: { in: { id: 65 }, out: { id: 40 } } },
      { date: "2026-08-14", teams: { in: { id: 1798 }, out: { id: 65 } } },
    ];
    check("the newest transfer INTO one of the two clubs decides it",
      pickCurrentClub(tx, opts)?.team, "Coventry");
    // Order in the response means nothing; the date does.
    check("...whatever order the feed lists them in",
      pickCurrentClub(tx.slice().reverse(), opts)?.team, "Coventry");
    check("...and it reads the same way round",
      pickCurrentClub([{ date: "2026-08-14", teams: { in: { id: 65 }, out: { id: 1798 } } },
                       { date: "2024-07-01", teams: { in: { id: 1798 } } }], opts)?.team,
      "Nottingham Forest");
    /* A move to a THIRD club is not one of the options -- between the two on
       offer the newest that IS one still beats the other. */
    check("a transfer somewhere else entirely is skipped, not chosen",
      pickCurrentClub([{ date: "2026-08-20", teams: { in: { id: 999 } } }, ...tx], opts)?.team,
      "Coventry");
    // No history, or none of it involving either club: nothing to say. The
    // caller keeps first-listed and reports that it could not check.
    check("no transfer history decides nothing", pickCurrentClub([], opts), null);
    check("...and neither does an absent one", pickCurrentClub(null, opts), null);
    check("history that never mentions either club decides nothing",
      pickCurrentClub([{ date: "2026-08-20", teams: { in: { id: 999 } } }], opts), null);
    // Rows the feed leaves half-filled must not throw or win.
    check("a malformed row is ignored rather than crashing",
     ...
     dropped him, so the pool keeps him at City. */
  {
    ...
    /* A club's transfer history is HISTORY. The newest row for a player is not
       the same thing as where he plays now -- the feed records arrivals far
       more completely than departures, so a loan that ended three seasons ago
       can still be the last thing on record about him.

       Observed on a live pull with no floor: 606 players "signed from outside
       the competition", among them Arthur at Liverpool on a loan from several
       seasons back, Milner at Brighton, Kurzawa at Fulham. Every one an old
       move read as a new one. */
    ...
    const old = [{ player: { id: 3, name: "Arthur" }, transfers: [
      { date: "2022-09-01", teams: { in: { id: 42 }, out: { id: 700 } } } ] }];
    check("a loan from three seasons ago is not news",
      Object.keys(latestTransfers(old, transferFloor(2026))).length, 0);
    check("...and is still read when no floor is asked for",
      Object.keys(latestTransfers(old)).length, 1);
    check("this season's move survives the floor",
      latestTransfers(rows, transferFloor(2026))[1]?.inId, 42);
    // A pre-season signing dated before the window formally opens is inside.
    ...
    /* The season floor was not enough, and a second live run showed why:
       J. Gelhardt was moved Hull City → Leeds, when he had gone Leeds → Hull
       permanently in August. The newest row on record for him was a RETURN
       FROM LOAN to Leeds dated 30 June -- the August move had not been
       ingested. So a transfer's date is when the move happened, not when the
       feed learned of it, and "the dated record is newer" is false.

       The squad list, though, is a snapshot with a timestamp. Anything dated
       before it is already baked in, whichever way the two disagree. */
    check("the floor is the last squad pull when that is the later of the two",
      transferSince(2026, "2026-08-20T09:00:00Z"), "2026-08-20");
    check("...and the season floor when the pull is older than it",
      transferSince(2026, "2025-11-02T09:00:00Z"), "2026-06-01");
    check("no earlier pull to compare against means acting on nothing",
      [transferSince(2026, null), transferSince(2026, ""), transferSince(2026, "rubbish")],
      [null, null, null]);
    // Gelhardt: squads pulled today, loan-return dated June. Already baked in.
    const gelhardt = [{ player: { id: 5, name: "J. Gelhardt" }, transfers: [
      { date: "2026-06-30", teams: { in: { id: 42 }, out: { id: 66 } } } ] }];
    check("a loan return older than the squad pull is not a move",
      Object.keys(latestTransfers(gelhardt, transferSince(2026, "2026-08-26"))).length, 0);
    check("...and the same record IS a move when the squads predate it",
      latestTransfers(gelhardt, transferSince(2026, "2026-06-01"))[5]?.inId, 42);

    // Squads: Villa still list Konsa, City still list Rodri, Arsenal have
    // neither. Exactly the state both bugs live in.
    const squads = [
    ...
    check("transfers: a move inside the competition is applied",
      r.players.find((p) => p.player_id === "api_1").team, "Arsenal");
    check("...and the club code and id follow, or crests and fixtures break",
      [r.players.find((p) => p.player_id === "api_1").team_code,
       r.players.find((p) => p.player_id === "api_1").team_logo], ["ARS", 42]);
    check("...and it is reported, not applied silently",
      r.moved.map((m) => [m.name, m.from, m.to]), [["E. Konsa", "Aston Villa", "Arsenal"]]);
    check("transfers: a move OUT of the competition removes him from the pool",
      r.players.some((p) => p.player_id === "api_2"), false);
    check("...and says who, and where he was",
      r.left.map((g) => [g.name, g.team]), [["Rodri", "Manchester City"]]);
    check("transfers: somebody with no transfer on record is untouched",
      r.players.find((p) => p.player_id === "api_3").team, "Arsenal");

    /* An arrival nobody's squad lists yet: not at his old club, not at his new
       one. Without the previous pool he simply vanishes, and whoever drafted
       him is told he has left the competition. */
    const gone = squads.filter((p) => p.player_id !== "api_1");
    const carried = applyTransfers(gone, squads, latest, teams);
    check("transfers: an unlisted arrival is carried forward from the old pool",
      carried.players.find((p) => p.player_id === "api_1")?.team, "Arsenal");
    check("...keeping the position his old row had, since the feed gives none",
      carried.players.find((p) => p.player_id === "api_1")?.position, "DEF");
    check("...and reported separately from an ordinary move",
      [carried.carried.length, carried.moved.length], [1, 0]);

    /* Signed from OUTSIDE the competition, and his new club has not listed him
       yet. Nothing to move and nothing to carry -- he has never been here --
       so he is handed back for a lookup rather than skipped in silence, which
       is what would leave the best free agent in the window undraftable. */
    const arriving = { 7: { date: "2026-08-18", inId: 42, outId: 500 } };
    const inbound = applyTransfers(squads, [], arriving, teams);
    check("transfers: a signing from another league is handed back to be looked up",
      inbound.unplaced.map((u) => [u.apiId, u.team]), [[7, "Arsenal"]]);
    check("...and is not mistaken for somebody leaving",
      [inbound.left.length, inbound.players.length], [0, squads.length]);
    check("transfers: a move between two clubs OUTSIDE the competition is nothing to us",
      applyTransfers(squads, [], { 7: { date: "2026-08-18", inId: 500, outId: 501 } },
        teams).unplaced.length, 0);

    /* The row built from that lookup. Position is the whole reason the call is
       made: the quota, the formation and every per-position rule read it, so a
       row without one is a player nobody can legally field. */
    const club = { name: "Arsenal", code: "ARS" };
    const api = { player: { id: 7, name: "A Signing", photo: "p.png" },
      statistics: [
        { team: { id: 500 }, games: { position: "Attacker", number: 9 } },
        { team: { id: 42 }, games: { position: "Midfielder", number: 8 } } ] };
    const row = arrivalRow(api, club, 42);
    check("arrival: the block for the club he JOINED decides his position",
      [row.position, row.number], ["MID", 8]);
    check("arrival: the id matches every other pool row's shape",
      [row.player_id, row.api_id, row.team, row.team_code], ["api_7", 7, "Arsenal", "ARS"]);
    check("arrival: a player who has only played elsewhere is still placeable",
      arrivalRow({ player: { id: 7, name: "X" },
        statistics: [{ team: { id: 500 }, games: { position: "Goalkeeper" } }] },
        club, 42).position, "GK");
    check("arrival: no position anywhere → no row, because it would be unfieldable",
      arrivalRow({ player: { id: 7, name: "X" }, statistics: [{ games: {} }] }, club, 42), null);
    check("arrival: an empty or malformed answer is not a crash",
      [arrivalRow(null, club, 42), arrivalRow({}, club, 42)], [null, null]);

    /* The last place in the pull where a missing value becomes a confident
       wrong answer. apiPosToSlot has always defaulted to MID and it has to
       keep doing so -- if the feed renamed "Attacker" to "Forward", refusing
       the unmapped ones would delete every forward in the competition -- so
       the pull asks knownApiPos() first and reports what it absorbed. */
    check("positions: the feed's own four words are known",
      ["Goalkeeper", "Defender", "Midfielder", "Attacker"].map(knownApiPos),
      [true, true, true, true]);
    check("positions: anything else is NOT, so it can be reported",
      ["Forward", "Wing-back", "", null, undefined].map(knownApiPos),
      [false, false, false, false, false]);
    check("positions: ...and still maps to MID rather than being dropped",
      ["Forward", null].map(apiPosToSlot), ["MID", "MID"]);
    check("positions: a word inherited from Object.prototype is not a position",
      [knownApiPos("constructor"), knownApiPos("toString")], [false, false]);

    /* Removals are capped. Emptying somebody's squad on the strength of a feed
       nobody here has ever seen a response from is the one mistake that costs
       a manager something. */
    ...
    for (let i = 0; i < MAX_TRANSFER_REMOVALS + 1; i++) {
      ...
      manyLatest[100 + i] = { date: "2026-08-0" + (i % 9) + "1", inId: 900, outId: 42 };
    }
    const capped = applyTransfers(many, [], manyLatest, teams);
    check("transfers: an implausible number of departures is not applied",
    ...
    /* And the same judgement about arrivals, with a bigger number: a window
       moves tens of players into a league, not hundreds. Past that the feed is
       describing history, and each lookup would spend an API call proving it. */
    const flood = {}, none = [];
    for (let i = 0; i < MAX_TRANSFER_ARRIVALS + 1; i++)
      flood[200 + i] = { date: "2026-08-01", inId: 42, outId: 900, name: "N" + i };
    const flooded = applyTransfers(none, [], flood, teams);
    check("transfers: an implausible number of arrivals is not looked up at all",
      [flooded.unplaced.length, flooded.flooded.length], [0, MAX_TRANSFER_ARRIVALS + 1]);
    const trickle = { 200: flood[200], 201: flood[201] };
    check("transfers: a believable number of arrivals still is",
      applyTransfers(none, [], trickle, teams).unplaced.length, 2);

    // ...but a believable number still is.
    const few = many.slice(0, 2), fewLatest = { 100: manyLatest[100], 101: manyLatest[101] };
    check("transfers: a believable number of departures still applies",
      applyTransfers(few, [], fewLatest, teams).players.length, 0);
  }

  /* The read-only squad check: what the STORED pool says, pulling nothing.
     The refresh reports what a pull CHANGED; this answers the question you
     actually have when a refresh looks like it did nothing. */
  {
    const picks = [
      { slot: "MID", player_id: "api_1", player_name: "Stayed", team: "City" },
      { slot: "DEF", player_id: "api_2", player_name: "Moved", team: "City" },
      { slot: "FWD", player_id: "api_3", player_name: "Left", team: "City" },
      { slot: "TEAM", player_id: "ENG", player_name: "England", team: "England" },
      { slot: "MID", player_id: null, player_name: "Empty slot", team: null },
    ];
    const pool = { api_1: { team: "City" }, api_2: { team: "Spurs" } };
    const c = squadCheck(picks, pool);
    check("squad check: club picks and empty slots are not players", c.total, 3);
    check("squad check: a pick the pool agrees with is not reported",
      [c.moved.map((m) => m.name), c.gone.map((g) => g.name)], [["Moved"], ["Left"]]);
    check("squad check: a disagreement names both clubs",
      [c.moved[0].from, c.moved[0].to], ["City", "Spurs"]);
    check("squad check: a leaver is reported with the club he last played for",
      c.gone[0].team, "City");
    check("squad check: no pool at all means nobody can be placed",
      squadCheck(picks, {}).gone.length, 3);
    check("squad check: an empty squad is not an error",
      [squadCheck([], pool).total, squadCheck(null, pool).gone], [0, []]);
  }
  ...
  /* How stale the shared squad list is. Nothing refreshes it on a schedule --
     the daily cron writes competition_stats and never touches
     competition_pools -- so without saying it out loud a league's clubs stay
     frozen at first load for the whole season and nobody can tell. */
  {
    const now = Date.UTC(2026, 7, 25, 9, 0);
    const ago = (d) => new Date(now - d * 864e5).toISOString();
    check("a pull today is not stale", poolAge(ago(0), now).stale, false);
    check("...and says so plainly", poolAge(ago(0), now).text, "Squads last pulled today.");
    check("yesterday reads as yesterday", poolAge(ago(1), now).text,
      "Squads last pulled yesterday.");
    check("just inside the window is still fine",
      [poolAge(ago(POOL_STALE_DAYS - 1), now).stale,
       poolAge(ago(POOL_STALE_DAYS - 1), now).days], [false, POOL_STALE_DAYS - 1]);
    check("a week is stale, and names the consequence",
      [poolAge(ago(POOL_STALE_DAYS), now).stale,
       /transfer/.test(poolAge(ago(POOL_STALE_DAYS), now).text)], [true, true]);
    check("a month is stale and counts the days",
      poolAge(ago(34), now).text,
      "Squads last pulled 34 days ago — any real-world transfer since then is not in the app yet.");
    // Never stamped: pulled before updated_at existed. That is old, not new.
    check("no timestamp at all is treated as stale", poolAge(null, now).stale, true);
    check("...and does not claim a number of days", poolAge(null, now).days, null);
    check("an unparseable timestamp is stale too", poolAge("not a date", now).stale, true);
    // A clock skewed the wrong way must not report a negative age.
    check("a pull from the future reads as today", poolAge(ago(-3), now).days, 0);
  }
