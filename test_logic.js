...
const { S, pickInfo, myManager, isAdmin, boardRulesNote, calcPlayerPoints, calcTeamPoints, computeScores,
        ...
        h2hSchedulePlan, rumblePlacement, matchdayPlan, fmtCountdown, roundRecap, deadlineCrossed, managerStreaks, seasonAwards, picksUntilTurn, autoPickPreview, navGroups, groupOfTab, isCupCompetition, CREATE_PRESETS, scoringBalance, pointsHistogram, statSummary, rowPointsWith,
        buildFixtureStatRows, mergeFixtureRows,
        fixtureWindows, matchweeksOf, WINDOW_DEFAULTS, shortlistCoverage, coverageHint, applyVisibleOrder,
        currentSeasonFor, seasonOptions, seasonLabel, backtestSeason, pickReconciliation, squadCheck, clubEvidence,
...
/* A pull must not replace what is known with less than was known.

   Both symptoms came from the same place. Goals conceded reverted to the
   club's whole-match total whenever the optional events call failed, because
   the fallback row was written straight over the good one -- so a manual
   re-pull was needed to put it back, and THAT re-pull recomputed man of the
   match off ratings the feed had revised in the meantime, moving a forward's
   award to a goalkeeper and taking three points off a settled round. */
{
  const row = (pid, motm, conc, cs) => ({
    player_id: pid, motm, clean_sheet: cs,
    raw: { motm: motm ? 1 : 0, "goals.conceded": conc, "shots.on": 2 },
  });
  const stored = [row("a", true, 1, false), row("b", false, 0, true)];

  // Ratings moved overnight and this pull would hand the award to somebody else.
  const moved = [row("a", false, 1, false), row("b", true, 0, true)];
  const kept = mergeFixtureRows(moved, stored, true);
  check("pull: an award already made does not move",
    kept.map((r) => [r.player_id, r.motm]), [["a", true], ["b", false]]);
  check("pull: ...and the raw copy of it agrees with the column",
    kept.map((r) => r.raw.motm), [1, 0]);

  // No events: the club total would flatten a figure derived from more.
  const flat = [row("a", false, 2, false), row("b", false, 2, false)];
  const held = mergeFixtureRows(flat, stored, false);
  check("pull: no events keeps the conceded figures already stored",
    held.map((r) => r.raw["goals.conceded"]), [1, 0]);
  check("pull: ...and the clean sheet that hangs off the same number",
    held.map((r) => r.clean_sheet), [false, true]);

  /* The correction path, which everything else must not break: a pull WITH
     events replaces a stored club-total fallback. */
  const better = mergeFixtureRows([row("a", false, 0, true)], stored, true);
  check("pull: events still correct a stored fallback",
    [better[0].raw["goals.conceded"], better[0].clean_sheet], [0, true]);

  // Nothing stored: this pull is the only thing that knows anything.
  const first = mergeFixtureRows(moved, [], true);
  check("pull: the first write of a fixture is taken as it comes",
    first.map((r) => r.motm), [false, true]);
  check("pull: ...and an absent prior list is the same thing",
    mergeFixtureRows(moved, null, false).map((r) => r.motm), [false, true]);

  // A player who was not in the stored rows at all (a late mapping fix).
  const added = mergeFixtureRows([...moved, row("c", true, 1, false)], stored, true);
...
/* A player tells you he has moved by playing for somebody else.

   Squad lists lag -- Rodri sat in Manchester City's for nine days after
   signing for Barcelona -- so a refresh built from them cannot see a transfer
   the feed has not caught up with. But competition_stats stamps the club each
   row was played FOR, so a player who has featured for his new side has
   already said so, at no extra cost and with no endpoint to disbelieve.

   The guard is the whole of the safety: only a match NEWER than the pool pull
   counts. Reading an older one walks a player backwards into the club he has
   just left, which is what a 30-June loan return did to J. Gelhardt. */
{
  const pool = [
    { player_id: "a", name: "O. Marmoush", team: "Manchester City" },
    { player_id: "b", name: "Stayer", team: "Arsenal" },
    { player_id: "c", name: "No Stats", team: "Everton" },
  ];
  const row = (pid, team, date, appeared = true) => ({
    player_id: pid, team, appeared, match_label: `X vs Y (${date})` });
  const pulled = Date.parse("2026-08-20T00:00:00Z");

  const found = clubEvidence(pool, [
    row("a", "Manchester City", "2026-08-10"),
    row("a", "Tottenham", "2026-08-30"),
    row("b", "Arsenal", "2026-08-30"),
  ], pulled);
  check("evidence: a player who turned out elsewhere is reported",
    found.map((f) => [f.name, f.pool, f.played, f.when]),
    [["O. Marmoush", "Manchester City", "Tottenham", "2026-08-30"]]);
  check("evidence: somebody at the club the pool says is not reported",
    found.some((f) => f.player_id === "b"), false);
  check("evidence: no stats at all says nothing",
    found.some((f) => f.player_id === "c"), false);

  /* The Gelhardt guard. His newest row was a loan return dated before the
     pull; acting on it moved him to the club he had just left. */
  check("evidence: a match older than the pool pull is not news",
    clubEvidence(pool, [row("a", "Tottenham", "2026-08-10")], pulled), []);
  /* A match on the SAME DAY as the pull is ambiguous -- only the date is
     known, not the kick-off -- and ambiguity errs toward telling you. This is
     a report, not an automatic write, and a transfer missed in silence is the
     failure this exists to end. */
  check("evidence: the same day as the pull is reported, not swallowed",
    clubEvidence(pool, [row("a", "Tottenham", "2026-08-20")], pulled).length, 1);
  check("evidence: with no pull date on record, the match still counts",
    clubEvidence(pool, [row("a", "Tottenham", "2026-08-10")], null).length, 1);

  // The newest row wins, even when an older one agrees with the pool.
  check("evidence: it is the LATEST club he played for that counts",
    clubEvidence(pool, [row("a", "Tottenham", "2026-08-25"),
                        row("a", "Manchester City", "2026-08-30")], pulled), []);
  check("evidence: a row he did not appear in proves nothing",
    clubEvidence(pool, [row("a", "Tottenham", "2026-08-30", false)], pulled), []);
  check("evidence: an undated label cannot be ordered, so it is ignored",
    clubEvidence(pool, [{ player_id: "a", team: "Tottenham", appeared: true,
                          match_label: "X vs Y" }], pulled), []);
  check("evidence: nothing anywhere is not a crash",
...
/* Waiver-order free-agent claims (mechanics-notes §1). */
{
  ...
  /* The audit trail. The resolver has always worked the contest out -- it is
     what decides whether the winner keeps his turn -- and used to throw the
     answer away, so a week later nobody could say whether a claim was won or
     merely unopposed. It is not recoverable afterwards: "who else listed him"
     is a broader set than this test, and the queue has moved on. */
  check("audit: a contested win records the rival who lost",
    r2.contest.a, { contested: true, rivals: ["M2"], pos: 1 });
  check("audit: the seat is the one he claimed FROM, before the win moved him",
    [r2.contest.a.pos, r2.order.M1], [1, 2]);
  {
    const solo = [{ id: "s", manager_id: "M2", rank: 0, out_player_id: "O2",
                    in_player_id: "P9", pick_id: "pk2" }];
    const r = resolveFaClaims(solo, { M1: 0, M2: 1 }, ["O1", "O2"], Infinity,
      { pk1: "O1", pk2: "O2" });
    check("audit: an unopposed win says so, with nobody named",
      r.contest.s, { contested: false, rivals: [], pos: 2 });
    check("audit: ...and only awarded claims carry an entry",
      Object.keys(r.contest), ["s"]);
  }
  {
    /* The narrow test, audited. A rival who has already spent the slot his
       claim would drop from was never really in the running, and must not be
       recorded as having been beaten -- the same rule the resolver uses to
       decide the turn, so the trail cannot contradict the demotion. */
    const cl = [
      { id: "a1", manager_id: "A", rank: 0, out_player_id: "O1", in_player_id: "X", pick_id: "pA" },
      { id: "a2", manager_id: "A", rank: 1, out_player_id: "O1", in_player_id: "W", pick_id: "pA" },
      { id: "c1", manager_id: "C", rank: 0, out_player_id: "O3", in_player_id: "W", pick_id: "pC" },
    ];
    const r = resolveFaClaims(cl, { A: 0, C: 1 }, ["O1", "O3"], 2, { pA: "O1", pC: "O3" });
    check("audit: a rival who could never have fired is not recorded as beaten",
      r.contest.c1, { contested: false, rivals: [], pos: 2 });
    check("audit: ...which agrees with C keeping his place", r.order.C, 1);
  }
